...
void SceneLighting(
  in float3 worldPos, in float3 normal,
  ...
  for (uint i = 0; i < lights.numLights; i++)
  {
    ...
    case AMBIENT:
    {
      ambientL.Compute(lights.atts[i], normal, viewDir, worldPos, albedoColor, f0, f90, roughness, diffuseLightContribution, specularLightContribution);
    }
    break;
    case DIRECTIONAL:
    {
