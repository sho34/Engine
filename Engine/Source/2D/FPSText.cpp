...
void FPSText::Destroy()
{
	textBrush = nullptr;
	textFormat = nullptr;
}

void FPSText::Render(ComPtr<ID2D1DeviceContext5> d2d1DeviceContext, uint32_t fps) {
	d2d1DeviceContext->BeginDraw();
	d2d1DeviceContext->SetTransform(D2D1::Matrix3x2F::Identity());

	D2D1_RECT_F Wnd{};

	Wnd = D2D1::RectF(static_cast<FLOAT>(10), static_cast<FLOAT>(10), static_cast<FLOAT>(250), static_cast<FLOAT>(30));
